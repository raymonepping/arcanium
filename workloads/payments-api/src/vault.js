// vault.js — Vault client for workloads.
// AppRole login + token refresh. No DB credential management.
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = httpsRequest({
      ...
      res.on('end', () => {
        ...
        try { resolve(ct.includes('application/json') ? JSON.parse(text) : text) }
        catch { resolve(text) }
      })
    })
    req.on('error', reject)
...
export async function init() { await login() }
export function getToken()   { return state.token }
export function isAuthenticated() { return state.authenticated }
