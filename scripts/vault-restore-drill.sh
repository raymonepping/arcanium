#!/usr/bin/env bash
# scripts/vault-restore-drill.sh — Prompt 24, Deliverable 3 (Drill A).
#
# Proves recovery, doesn't assert it: takes a real snapshot, restores it
# into a genuinely separate, isolated Vault instance (own compose project,
# own fresh volume — compose/vault/compose.restore-drill.yaml), verifies
# real post-restore state (a known Transit key's version, a known
# namespace), measures real RTO/RPO, records the result, and tears the
# isolated instance down. "Snapshot is readable" is never treated as
# "restore succeeded."
set -uo pipefail
umask 077
...
PROJECT="arcanium-vault-restore-drill"
