...
function mockQuery(text, params = []) {
  ...
  if (t.startsWith('INSERT INTO APPLICATIONS')) {
    ...
    const row = { id: `00000000-0000-0000-0000-${String(db.nextId++).padStart(12,'0')}`, name, description, registered_at: new Date().toISOString() }
    db.rows.push(row)
    return { rows: [row] }
  }
...
function buildApp() {
  ...
  router.get('/', async (_req, res, next) => {
    try { res.json((await mockQuery('SELECT * FROM applications ORDER')).rows) } catch(err){next(err)}
  })
  router.post('/', async (req, res, next) => {
    try {
      ...
      const result = await mockQuery('INSERT INTO applications', [name, description ?? null])
  ...
  router.get('/:id', async (req, res, next) => {
    try {
      ...
      const apps = (await mockQuery('SELECT * FROM applications WHERE id=$1', [req.params.id])).rows
      if (!apps.length) return res.status(404).json({ error: 'not found' })
      const profiles = (await mockQuery('SELECT * FROM crypto_profiles WHERE application_id=$1', [req.params.id])).rows
      res.json({ ...apps[0], crypto_profiles: profiles })
    } catch(err){next(err)}
  })
  router.delete('/:id', async (req, res, next) => {
    try {
      const r = await mockQuery('DELETE FROM applications WHERE id=$1', [req.params.id])
...
test('POST /api/v1/applications with valid body returns 201', async () => {
  db.rows = []
  const app = buildApp()
  const { status, body } = await req(app, 'POST', '/api/v1/applications', { name: 'payments-api', description: 'test' })
...
test('POST /api/v1/applications with missing name returns 400', async () => {
  const app = buildApp()
  const { status, body } = await req(app, 'POST', '/api/v1/applications', { description: 'no name' })
...
test('POST /api/v1/applications with invalid name returns 400', async () => {
  const app = buildApp()
  const { status, body } = await req(app, 'POST', '/api/v1/applications', { name: 'invalid name!' })
  assert.equal(status, 400)
  assert.equal(body.field, 'name')
})
...
test('POST /api/v1/applications duplicate name returns 409', async () => {
  ...
  const { status, body } = await req(app, 'POST', '/api/v1/applications', { name: 'my-app' })
...
test('GET /api/v1/applications/:id with unknown UUID returns 404', async () => {
  db.rows = []
  const app = buildApp()
  const { status } = await req(app, 'GET', '/api/v1/applications/00000000-0000-0000-0000-000000000099')
...
test('DELETE /api/v1/applications/:id returns 204', async () => {
  db.rows = []
  const app = buildApp()
  const { body: created } = await req(app, 'POST', '/api/v1/applications', { name: 'to-delete' })
  const { status } = await req(app, 'DELETE', `/api/v1/applications/${created.id}`)
