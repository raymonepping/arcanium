...
async function loginWithBackoff(fetcher, maxAttempts = 5) {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    try {
      ...
      lastErr = err
      if (i < maxAttempts - 1) await new Promise(r => setTimeout(r, delays[i]))
    }
  }
  throw new Error(`login exhausted after ${maxAttempts} attempts: ${lastErr.message}`)
}
...
test('login throws after exhausting all retries', async () => {
  const fetcher = async () => { throw new Error('vault unavailable') }
