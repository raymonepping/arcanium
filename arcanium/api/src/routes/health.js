...
healthRouter.get('/ready', async (_req, res) => {
  ...
  if (!vaultState.authenticated) {
    return res.status(503).json({ status: 'unavailable', reason: 'vault not authenticated' })
  }
  try {
    await ping()
    res.json({ status: 'ready' })
  } catch (err) {
    res.status(503).json({ status: 'unavailable', reason: `database unreachable: ${err.message}` })
  }
})
